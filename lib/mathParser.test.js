...
test('toJSExpression: √2 → Math.sqrt(2)', () => {
  assertEqual(toJSExpression('√2'), 'Math.sqrt(2)')
})
...
test('toJSExpression: implicitní násobení (10-1)3 → (10-1)*3', () => {
  assertEqual(toJSExpression('(10-1)3'), '(10-1)*3')
  assertEqual(toJSExpression('2+(10-1)3'), '2+(10-1)*3')
})

test('toJSExpression: implicitní násobení (2+3)(4+5) → (2+3)*(4+5)', () => {
  assertEqual(toJSExpression('(2+3)(4+5)'), '(2+3)*(4+5)')
})
...
test('numeric: string expected value "1/6" accepts 2/12', () => {
  const result = evaluateAnswer('2/12', {
    question: {},
    answer: { type: 'numeric', value: '1/6', unit: null }
  })
  assertTrue(result.isCorrect)
})

test('numeric: string expected value "3/4" accepts 0,75', () => {
  const result = evaluateAnswer('0,75', {
    question: {},
    answer: { type: 'numeric', value: '3/4', unit: null }
  })
  assertTrue(result.isCorrect)
})

test('numeric: string expected value "2/3" accepts 4/6', () => {
  const result = evaluateAnswer('4/6', {
    question: {},
    answer: { type: 'numeric', value: '2/3', unit: null }
  })
  assertTrue(result.isCorrect)
})
