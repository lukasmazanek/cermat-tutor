...
export function toJSExpression(input) {
  ...
  // √ followed by number: √2 → Math.sqrt(2)
  ...
  // Implicitní násobení: )2 → )*2, )( → )*(
  // Např. (10-1)3 → (10-1)*3, (2+3)(4+5) → (2+3)*(4+5)
...
export function evaluateAnswer(userInput, problem) {
  ...
  // Infer type if not provided: if value contains 'x', it's symbolic
  let type = answer.type
  if (!type) {
    const isSymbolic = typeof expectedValue === 'string' &&
      expectedValue.toLowerCase().includes('x')
  ...
  if (type === 'numeric') {
    ...
    // Evaluate expected value if it's a string expression (e.g., "1/6")
    ...
    const valueCorrect = numbersEqual(userResult, expectedNumeric)

    // Zkontrolovat jednotku (pokud vyžadována)
    if (expectedUnit && !foundUnit) {
      // Chybí jednotka - vždy vyžadujeme
